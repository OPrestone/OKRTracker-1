...
export const objectiveStatusReasonEnum = pgEnum("objective_status_reason", [
	"success",
	"failed",
	"changed",
	"other",
...
export const strategicDirectionTypeEnum = pgEnum("strategic_direction_type", [
	"company",
	"team",
]);
...
export const organizationMission = pgTable("organization_mission", {
	id: text("id")
		.primaryKey()
		.$defaultFn(() => ulid()),
...
export const teams = pgTable("teams", {
	...
	color: text("color"),
	icon: text("icon"),
	parentId: text("parent_id").references((): AnyPgColumn => teams.id, {
		onDelete: "set null",
	}),
...
export const cadences = pgTable("cadences", {
	...
	period: text("period").notNull(), // e.g., 'weekly', 'quarterly'
...
export const objectives = pgTable("objectives", {
	...
	parentId: text("parent_id").references((): AnyPgColumn => objectives.id, {
		onDelete: "set null",
	}),
	strategyId: text("strategy_id").references(() => strategicDirections.id),
	// New field for alignment: team OKRs can link to company goals
	alignmentTargetId: text("alignment_target_id").references(
...
export const userBadges = pgTable("user_badges", {
	...
	awardedById: text("awarded_by_id")
		.references(() => users.id)
		.notNull(),
	reason: text("reason"),
	tenantId: text("tenant_id")
...
// Strategic Intent table - simplified version
export const strategicIntents = pgTable("strategic_intents", {
	id: text("id")
		.primaryKey()
		.$defaultFn(() => ulid()),
	title: text("title").notNull(),
	duration: integer("duration").notNull(), // 1-5 years
	priority: integer("priority"),
	type: text("type"),
	tenantId: text("tenant_id")
		.references(() => tenants.id)
	...
	teamId: text("team_id").references(() => teams.id),
	createdById: text("created_by_id").references(() => users.id),
	parentDirectionId: text("parent_direction_id").references(
		(): AnyPgColumn => strategicIntents.id,
		{ onDelete: "set null" }
	),
	createdAt: timestamp("created_at").defaultNow(),
	updatedAt: timestamp("updated_at").defaultNow(),
...
export const strategicDirections = strategicIntents;
export const strategicDirectionsTable = strategicIntents;

export const strategicIntentsRelations = relations(
	strategicIntents,
	({ one }) => ({
		tenant: one(tenants, {
			fields: [strategicIntents.tenantId],
			references: [tenants.id],
		}),
		createdBy: one(users, {
			fields: [strategicIntents.createdById],
			references: [users.id],
		}),
	})
);

export const strategicDirectionsRelations = strategicIntentsRelations;
...
export const userAccessGroupsRelations = relations(
	userAccessGroups,
	({ one }) => ({
		user: one(users, {
...
export const financialBudgetsRelations = relations(
	financialBudgets,
	({ one }) => ({
		team: one(teams, {
...
export const insertUserToTenantSchema = createInsertSchema(usersToTenants).omit(
	{
		id: true,
		createdAt: true,
		updatedAt: true,
...
export const insertFinancialBudgetSchema = createInsertSchema(
	financialBudgets
).omit({ id: true, createdAt: true, updatedAt: true });
...
//     )
//   });
export const insertStrategicIntentSchema = createInsertSchema(
	strategicIntents
).omit({ id: true, createdAt: true, updatedAt: true });
export const insertStrategicDirectionSchema = insertStrategicIntentSchema; // Backward compatibility
export const insertOrganizationMissionSchema = createInsertSchema(
	organizationMission
).omit({ id: true, createdAt: true, updatedAt: true });
...
export type InsertStrategicDirection = z.infer<
	typeof insertStrategicDirectionSchema
>;
...
export type InsertOrganizationMission = z.infer<
	typeof insertOrganizationMissionSchema
>;
...
export const meetingsToObjectives = pgTable("meetings_to_objectives", {
	id: text("id")
		.primaryKey()
		.$defaultFn(() => ulid()),
	meetingId: text("meeting_id")
...
export const meetingsToKeyResults = pgTable("meetings_to_key_results", {
	id: text("id")
		.primaryKey()
		.$defaultFn(() => ulid()),
	meetingId: text("meeting_id")
...
export const meetingsToObjectivesRelations = relations(
...
export const meetingsToKeyResultsRelations = relations(
...
export const insertMeetingToUserSchema = createInsertSchema(
	meetingsToUsers
).omit({ id: true });
...
export const insertMeetingToObjectiveSchema = createInsertSchema(
	meetingsToObjectives
).omit({ id: true });
export type MeetingToObjective = typeof meetingsToObjectives.$inferSelect;
export type InsertMeetingToObjective = z.infer<
	typeof insertMeetingToObjectiveSchema
>;

export const insertMeetingToKeyResultSchema = createInsertSchema(
	meetingsToKeyResults
).omit({ id: true });
export type MeetingToKeyResult = typeof meetingsToKeyResults.$inferSelect;
export type InsertMeetingToKeyResult = z.infer<
	typeof insertMeetingToKeyResultSchema
>;

export const insertActionItemSchema = createInsertSchema(actionItems);

// OKR System Configurations
export const okrSystemConfigs = pgTable("okr_system_configs", {
	id: text("id")
		.primaryKey()
		.$defaultFn(() => ulid()),
	...
	max_objectives_per_team: integer("max_objectives_per_team")
		.default(5)
		.notNull(),
	max_key_results_per_objective: integer("max_key_results_per_objective")
		.default(3)
		.notNull(),
	...
	enable_quarterly_cadence: boolean("enable_quarterly_cadence")
		.default(true)
		.notNull(),
	enable_annual_cadence: boolean("enable_annual_cadence")
		.default(true)
		.notNull(),
	custom_cadence: text("custom_cadence"),
	default_objective_category: text("default_objective_category")
		.default("growth")
		.notNull(), // growth, product, customer, people, financial, operations, other
	require_objective_approval: boolean("require_objective_approval")
		.default(true)
		.notNull(),
	enable_objective_alignment: boolean("enable_objective_alignment")
		.default(true)
		.notNull(),
	enable_cross_team_objectives: boolean("enable_cross_team_objectives")
		.default(true)
		.notNull(),
	enable_slack_integration: boolean("enable_slack_integration")
		.default(false)
		.notNull(),
	enable_email_notifications: boolean("enable_email_notifications")
		.default(true)
		.notNull(),
	enable_calendar_sync: boolean("enable_calendar_sync").default(false).notNull(),
	enable_analytics_reporting: boolean("enable_analytics_reporting")
		.default(true)
		.notNull(),
...
export type OkrSystemConfig = typeof okrSystemConfigs.$inferSelect;
export type InsertOkrSystemConfig = z.infer<
	typeof okrSystemConfigs.$inferInsert
>;
