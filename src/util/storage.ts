...
export interface IStorage {
	...
	getUserProgressByUserAndObjective(
		userId: string,
		objectiveId: string
	): Promise<UserProgress | undefined>;
	getUserObjectivesProgress(
		userId: string,
		tenantId: string
	...
	getMeetingWithDetails(id: string): Promise<
		| (Meeting & {
	...
	getMeetingAttendees(
		meetingId: string
	): Promise<Array<User & { isAttending: boolean }>>;
	...
	addObjectiveToMeeting(
		meetingId: string,
		objectiveId: string
	): Promise<MeetingToObjective>;
	removeObjectiveFromMeeting(
		meetingId: string,
		objectiveId: string
	): Promise<void>;
	addKeyResultToMeeting(
		meetingId: string,
		keyResultId: string
	): Promise<MeetingToKeyResult>;
	removeKeyResultFromMeeting(
		meetingId: string,
		keyResultId: string
	): Promise<void>;
	...
	updateAccessGroup(
		id: string,
		accessGroup: Partial<InsertAccessGroup>
	): Promise<AccessGroup>;
	...
	getTimeframesWithObjectives(
		tenantId: string
	): Promise<Array<Timeframe & { objectives: Objective[] }>>;
	...
	updateObjective(
		id: string,
		objective: Partial<InsertObjective>
	): Promise<Objective>;
	...
	updateKeyResult(
		id: string,
		keyResult: Partial<InsertKeyResult>
	): Promise<KeyResult>;
	...
	updateInitiative(
		id: string,
		initiative: Partial<InsertInitiative>
	): Promise<Initiative>;
	...
	updateChatRoom(
		id: string,
		chatRoom: Partial<InsertChatRoom>
	): Promise<ChatRoom>;
	getAllChatRooms(): Promise<ChatRoom[]>;
	getChatRoomsByUser(userId: string): Promise<ChatRoom[]>;
	getUserChatRooms(
		userId: string
	): Promise<(ChatRoom & { unreadCount: number })[]>;
	...
	getMessagesByChatRoom(
		...
		(Message & { user: User; attachments: Attachment[]; reactions: Reaction[] })[]
	>;
...
export class DatabaseStorage implements IStorage {
	...
	async createUserWithTransaction(tx: any, userData: InsertUser): Promise<User> {
		try {
			console.log("Creating user with transaction - data:", {
				...userData,
				password: "***",
			});
	...
	async addUserToTeamWithTransaction(
		tx: any,
		userId: string,
		teamId: string
	): Promise<User> {
	...
	async getTeamsByParent(parentId: string): Promise<Team[]> {
		return db
			...
			.from(teams)
			.where(eq(teams.parentId, parentId));
	}
	...
	async getTeamPerformance(teamId: string, tenantId: string): Promise<any> {
		try {
			...
			const completedObjectives = teamObjectives.filter(
				(obj) => obj.progress === 100
			).length;
			const onTrackObjectives = teamObjectives.filter(
				(obj) => obj.progress >= 70 && obj.progress < 100
			).length;
			const atRiskObjectives = teamObjectives.filter(
				(obj) => obj.progress >= 40 && obj.progress < 70
			).length;
			const behindObjectives = teamObjectives.filter(
				(obj) => obj.progress < 40
			).length;

			// Calculate key result metrics
			const totalKeyResults = keyResultsForTeam.length;
			const completedKeyResults = keyResultsForTeam.filter(
				(kr) => kr.progress === 100
			).length;
			...
			const overallProgress =
				totalObjectives > 0
					? Math.round(
							teamObjectives.reduce((sum, obj) => sum + (obj.progress || 0), 0) /
								totalObjectives
					  )
					: 0;
			...
			const performance = {
				...
				objectives: teamObjectives.map((obj) => {
					...
					const objKeyResults = keyResultsForTeam.filter(
	...
	async getTeamMemberPerformance(
		...
		try {
			...
			const userKeyResults = await db
				...
				.from(keyResults)
				.where(
					and(eq(keyResults.assignedToId, userId), eq(keyResults.tenantId, tenantId))
				);
			...
			const completedObjectives = userObjectives.filter(
				(obj) => obj.progress === 100
			).length;
			const onTrackObjectives = userObjectives.filter(
				(obj) => obj.progress >= 70 && obj.progress < 100
			).length;
			const atRiskObjectives = userObjectives.filter(
				(obj) => obj.progress >= 40 && obj.progress < 70
			).length;
			const behindObjectives = userObjectives.filter(
				(obj) => obj.progress < 40
			).length;

			const totalAssignedKeyResults = keyResultsForTeamObjectives.length;
			const completedKeyResults = keyResultsForTeamObjectives.filter(
				(kr) => kr.progress === 100
			).length;

			// Calculate overall progress
			const overallProgress =
				totalAssignedObjectives > 0
					? Math.round(
							userObjectives.reduce((sum, obj) => sum + (obj.progress || 0), 0) /
								totalAssignedObjectives
					  )
					: totalAssignedKeyResults > 0
					? Math.round(
							keyResultsForTeamObjectives.reduce(
			...
			return {
				...
				objectives: userObjectives.map((obj) => {
					// Find key results for this objective
					const objKeyResults = userKeyResults.filter(
						(kr) => kr.objectiveId === obj.id
					);
			...
			console.error(
				`Error getting performance for team member ${userId} in team ${teamId}:`,
				error
			);
			throw error;
		}
	 ...
	 */
	async getTeamMembersPerformance(
		teamId: string,
		tenantId: string
	): Promise<any[]> {
		try {
			...
			const membersPerformance = await Promise.all(
				teamMembers.map(async (member) => {
					try {
						const performance = await this.getTeamMemberPerformance(
							teamId,
							member.id,
							tenantId
						);
						...
						console.error(
							`Error getting performance for team member ${member.id}:`,
							error
						);
						return {
							id: member.id,
			...
			console.error(
				`Error getting team members performance for team ${teamId}:`,
				error
			);
			return [];
		}
	...
	async createAccessGroup(accessGroup: InsertAccessGroup): Promise<AccessGroup> {
		try {
			...
			console.log(
				"Creating access group with data:",
				JSON.stringify(accessGroupToInsert)
			);

			// Only select columns that exist in the database table
			const [newAccessGroup] = await db
				.insert(accessGroups)
	...
	async updateAccessGroup(
		...
		const [updatedAccessGroup] = await db
			.update(accessGroups)
			.set(accessGroup)
			.where(eq(accessGroups.id, id))
			.returning();
	...
	async createTimeframe(timeframe: InsertTimeframe): Promise<Timeframe> {
		...
		if (timeframe.cadenceId) {
			...
			if (
				"tenantId" in timeframe &&
				cadence.tenantId &&
				timeframe.tenantId !== cadence.tenantId
			) {
				console.warn(
					`Warning: Timeframe tenant (${timeframe.tenantId}) doesn't match cadence tenant (${cadence.tenantId})`
				);
	...
	async getTimeframesByTenant(tenantId: string): Promise<Timeframe[]> {
		try {
			...
			console.log(
				"No timeframes found directly by tenant ID, checking cadences..."
			);
			...
			const timeframesFromCadences = await db
				.select({
					id: timeframes.id,
					name: timeframes.name,
					description: timeframes.description,
	...
	async updateTimeframe(
		id: string,
		timeframe: Partial<InsertTimeframe>
	): Promise<Timeframe> {
		...
		if (timeframe.cadenceId) {
			...
			if ("tenantId" in existingTimeframe && "tenantId" in cadence) {
				...
				if (
					existingTenantId &&
					cadenceTenantId &&
					existingTenantId !== cadenceTenantId
				) {
		...
		if ("tenantId" in timeframe && "tenantId" in existingTimeframe) {
			if (
				timeframe.tenantId &&
				existingTimeframe.tenantId &&
				timeframe.tenantId !== existingTimeframe.tenantId
			) {
	...
	async deleteTimeframe(id: string, tenantId?: string): Promise<void> {
		...
		if (tenantId && timeframe.tenantId && timeframe.tenantId !== tenantId) {
			throw new Error(
				`Access denied: Timeframe does not belong to tenant ${tenantId}`
			);
		}
	...
	async updateObjective(
		...
		// Update
		await db.update(objectives).set(cleanedUpdate).where(eq(objectives.id, id));
	...
	async getObjectivesByOwner(ownerId: string): Promise<Objective[]> {
		...
		return db
			...
			.from(objectives)
			.where(eq(objectives.ownerId, ownerId));
	}
	...
	async getObjectivesByTimeframe(timeframeId: string): Promise<Objective[]> {
		...
		return db
			...
			.from(objectives)
			.where(eq(objectives.timeframeId, timeframeId));
	}
	...
	async getApprovedObjectives(tenantId: string): Promise<Objective[]> {
		try {
			...
			const count = await db
				...
				.from(objectives)
				.where(
					and(eq(objectives.tenantId, tenantId), eq(objectives.isApproved, true))
				);

			console.log(
				`Found ${
					count[0]?.count || 0
				} approved objectives with tenantId: ${tenantId}`
			);
			...
			console.log(
				`Retrieved ${results.length} approved objectives from the database`
			);
	...
	async approveObjective(id: string): Promise<Objective> {
		...
		const keyResultsList = await this.getKeyResultsByObjective(
			updatedObjective.id
		);
	...
	async updateObjectiveProgress(
		...
		// Update the progress
		await db.update(objectives).set({ progress }).where(eq(objectives.id, id));
	...
	async createKeyResult(keyResult: InsertKeyResult): Promise<KeyResult> {
		...
		try {
			...
			const results = await this.getKeyResultsByObjective(
			...
			if (results && results.length > 0) {
				...
				const averageProgress =
					results.length > 0 ? Math.round(totalProgress / results.length) : 0;
	...
	async updateKeyResultProgress(
		...
		if (updatedKeyResult.objectiveId) {
			const keyResults = await this.getKeyResultsByObjective(
	...
	async getCheckInsByUserId(
		...
		try {
			...
			return db
				.select()
				.from(checkIns)
				.where(and(eq(checkIns.userId, userId), eq(checkIns.tenantId, tenantId)))
				.orderBy(desc(checkIns.createdAt));
		} catch (error) {
			console.error(
				`Error getting check-ins for user ${userId} in tenant ${tenantId}:`,
				error
			);
			return [];
		}
	...
	async updateCheckIn(
		...
		if (updatedCheckIn.keyResultId && typeof checkInData.progress === "number") {
			await this.updateKeyResultProgress(
				updatedCheckIn.keyResultId,
				checkInData.progress
			);
		}
	...
	async getChatRoom(
		...
		if (tenantId) {
			const [room] = await db
				.select()
				.from(chatRooms)
				.where(and(eq(chatRooms.id, id), eq(chatRooms.tenantId, tenantId)));
			return room;
		} else {
	...
	async updateChatRoom(
		...
		let query = db.update(chatRooms).set({
			...chatRoom,
			updatedAt: new Date(),
		});
		...
		if (tenantId) {
			query = query.where(
				and(eq(chatRooms.id, id), eq(chatRooms.tenantId, tenantId))
			);
		} else {
		...
		if (!updatedRoom) {
			throw new Error(
				`Chat room with id ${id} not found or belongs to a different tenant`
			);
		}
	...
	async getAllChatRooms(tenantId?: string): Promise<ChatRoom[]> {
		...
		if (tenantId) {
			return db.select().from(chatRooms).where(eq(chatRooms.tenantId, tenantId));
		} else {
	...
	async getChatRoomsByUser(
		...
		if (tenantId) {
			return db
				...
				.from(chatRooms)
				.where(
					and(inArray(chatRooms.id, roomIds), eq(chatRooms.tenantId, tenantId))
				);
		} else {
			// For backward compatibility, return all if no tenant specified
			return db.select().from(chatRooms).where(inArray(chatRooms.id, roomIds));
		}
	}
	...
	async updateMessage(
		...
		if (!updatedMessage) {
			throw new Error(
				`Message with id ${id} not found or belongs to a different tenant`
			);
		}
	...
	> {
		...
		if (tenantId) {
			baseConditions = and(baseConditions, eq(messages.tenantId, tenantId));
		}

		// Create appropriate query based on whether 'before' is provided
		let messageResults;
		if (before !== undefined) {
			const fullConditions = and(baseConditions, lt(messages.id, before));
	...
	async getMeetingWithDetails(id: string): Promise<
		| (Meeting & {
				attendees: Array<User & { isAttending: boolean }>;
				attendeeIds: string[];
				relatedObjectives: Objective[];
	...
	async getMeetingsByTenant(
		tenantId: string
	): Promise<(Meeting & { attendeeIds?: string[] })[]> {
		...
		const meetingsWithAttendeeIds = await Promise.all(
			allMeetings.map(async (meeting) => {
				const attendeeIdsResult = await db
					.select({
						userId: meetingsToUsers.userId,
					})
					.from(meetingsToUsers)
	...
	async getMeetingsByUser(
		...
		const userMeetingsData = await db
			.select()
			.from(meetings)
	...
	async getMeetingsByStatus(
		...
		const allMeetings = await db
			.select()
			.from(meetings)
			.where(and(eq(meetings.tenantId, tenantId), eq(meetings.status, status)))
			.orderBy(desc(meetings.scheduledStartTime));
	...
	async deleteMeeting(id: string): Promise<void> {
		...
		await db
			.delete(meetingsToObjectives)
			.where(eq(meetingsToObjectives.meetingId, id));
		await db
			.delete(meetingsToKeyResults)
			.where(eq(meetingsToKeyResults.meetingId, id));
	...
	async removeAttendeeFromMeeting(
		meetingId: string,
		userId: string
	): Promise<void> {
		await db
			.delete(meetingsToUsers)
			.where(
				and(
					eq(meetingsToUsers.meetingId, meetingId),
					eq(meetingsToUsers.userId, userId)
	...
	async updateAttendeeStatus(
		...
		if (!updatedAttendee) {
			throw new Error(
				`Attendee record not found for meeting ${meetingId} and user ${userId}`
			);
		}
	...
	async completeActionItem(id: string): Promise<ActionItem> {
		const [completedActionItem] = await db
			.update(actionItems)
			.set({
				completed: true,
	...
	async removeObjectiveFromMeeting(
		meetingId: string,
		objectiveId: string
	): Promise<void> {
		await db
	...
	async removeKeyResultFromMeeting(
		meetingId: string,
		keyResultId: string
	): Promise<void> {
		await db
	...
	async getFeedbackForUser(userId: string, tenantId: string): Promise<any[]> {
		try {
			...
			const receivedFeedback = await db
				...
				.from(feedback)
				.where(
					and(eq(feedback.receiverId, userId), eq(feedback.tenantId, tenantId))
				)
					...
					return {
						...
						fromUser: sender
							? {
									...
									initials:
										sender.initials || sender.name?.substring(0, 2).toUpperCase() || "UN",
							  }
							: null,
			...
			console.error(
				`Error getting feedback for user ${userId} in tenant ${tenantId}:`,
				error
			);
			return [];
		}
	...
	async createProject(projectData: any): Promise<Project> {
		try {
			...
			if (!tenantId) {
				throw new Error(
					"tenant_id is required but was not provided in the project data"
				);
			}
			...
			const dbFields = {
				...
				assigned_to_id:
					projectData.assigned_to_id || projectData.assignedToId || null,
			...
			if (!dbFields.tenant_id) {
				throw new Error(
					`tenant_id is required but was null or undefined: ${JSON.stringify(
						dbFields
					)}`
				);
			}
			...
			try {
				console.log(
					"About to execute SQL insert with these fields:",
   ...
   `;
	...
	async getProjectsByStatus(
		...
		return await db
			.select()
			.from(projects)
			.where(and(eq(projects.status, status), eq(projects.tenantId, tenantId)))
 ...
 `,
	...
	async getUserProgressByUserAndObjective(
		userId: string,
		objectiveId: string
	): Promise<UserProgress | undefined> {
		try {
	...
	async getUserObjectivesProgress(
		userId: string,
		tenantId: string
	): Promise<UserProgress[]> {
		try {
			return await db
				...
				.from(userProgress)
				.where(
					and(eq(userProgress.userId, userId), eq(userProgress.tenantId, tenantId))
				);
		} catch (error) {
