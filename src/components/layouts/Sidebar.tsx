...
import { cn, getPathWithoutUlid } from "@/lib/utils";
...
interface SidebarProps {
	...
	userNPermissions: UserPermissions;
}

const Sidebar = ({ open, onOpenChange, userNPermissions }: SidebarProps) => {
	//   const [location] = useLocation();
	const pathname = usePathname();
	const { user, permissions } = userNPermissions;
	const userRole = permissions.getUserRole;

	const location = getPathWithoutUlid(pathname);
	...
	const getDashboardInfo = () => {
		...
		} else if (permissions.isTeamLeader) {
			return {
	...
	const okrPaths = ["my-okrs", "draft-okrs", "approved-okrs", "company-okrs"];
	const userManagementPaths = ["teams", "all-users", "users"];
	const reportPaths = [
		"activity-report",
		"alignment-report",
		"completion-report",
	...
	const isOkrPathActive = okrPaths.some(
		(path) =>
			locationPath === `/${path}` ||
			(locationPath.includes(`/organization/`) &&
				locationPath.includes(`/${path}`))
	);
	const isUserManagementPathActive = userManagementPaths.some(
		(path) =>
			locationPath === `/${path}` ||
			(locationPath.includes(`/organization/`) &&
				locationPath.includes(`/${path}`))
	);
	const isReportPathActive = reportPaths.some(
		(path) =>
			locationPath === `/${path}` ||
			(locationPath.includes(`/organization/`) &&
				locationPath.includes(`/${path}`))
	);
	const isConfigPathActive = configPaths.some(
		(path) =>
			locationPath === `/${path}` ||
			(locationPath.includes(`/organization/`) &&
				locationPath.includes(`/${path}`))
	...
	// 	const userRole = user.tenants?.find(
	...
	// 	if (tenantMatch && !numericMatch && !ulidDirectMatch) {
	// 		// Ensure we're not matching a numeric ID or ULID again
		...
		<div className="flex flex-col h-full bg-[#0f172a] text-gray-200 shadow-xl">
			<div className="p-5 border-b border-slate-800/70 bg-gradient-to-r from-blue-950 to-indigo-950">
				<h1 className="text-xl font-semibold flex items-center">
					...
					<span className="text-white font-bold tracking-wide">Pinnacle OKR</span>
				</h1>
			...
			<nav className="flex-1 overflow-y-auto py-4">
				...
				>
					<Link
						href={getLink("/quick-start-guide")}
						className="flex items-center w-full"
					>
				...
				{okrsExpanded &&
					(permissions.canCreateObjectives ||
						...
						<div className="pl-8 mt-1 mb-1 space-y-1 py-1 ml-4 mr-4 bg-indigo-950/30 rounded-lg">
							...
							{/* Team OKRs - Available to all users */}
							<div
								className={cn(
									"flex items-center mx-2 px-3 py-2 text-sm transition-all duration-200 rounded-md",
									location === "/my-okrs"
				...
				<div
					className={cn(
						...
						location === "/one-on-one-meetings" ||
							location.includes("/one-on-one-meetings")
							? "bg-gradient-to-r from-indigo-600 to-indigo-500 text-white shadow-md shadow-indigo-600/20"
							: "text-slate-200 hover:bg-indigo-900/30 hover:text-white"
					)}
				>
						...
						{/* {selectedTenant && (isCurrentUserAdmin() || user?.isAdmin) && (
							<div className="mb-2">
								...
								<div
									className={cn(
										"flex items-center py-2 px-3 text-sm transition-all duration-200 rounded-md mb-1",
										location.startsWith(`/${selectedTenant.id}`) &&
