...
import { Clock, Loader2, Plus, Target, TrendingUp } from "lucide-react";
...
} from "@/components/onboarding/SmartCreateObjectiveButton";
import { TenantContext } from "@/contexts/TenantContext";
import { UserContext } from "@/contexts/UserContext";
import { UserPermissions } from "@/lib/actions";
import { getAllKeyResults } from "@/lib/queries/key-results";
import { getAllObjectives } from "@/lib/queries/objectives";
import { getAllTimeframes } from "@/lib/queries/timeframes";
import { KeyResult, Objective, Tenant, Timeframe } from "@/util/schema";
...
export default function TeamOKRDisplay() {
	const [currentTab, setCurrentTab] = useState("active");
	const router = useRouter();
	const userNPermissions = useContext(UserContext) as UserPermissions;
	...
	// Fetch key results
	const { data: keyResults = [], isLoading: loadingKeyResults } = useQuery<
		KeyResult[]
	>({
		queryKey: ["key-results", tenant.id],
		queryFn: async () => {
			const keyResults = await getAllKeyResults(tenant.id);
	...
	const { data: timeframes = [], isLoading: loadingTimeframes } = useQuery<
		Timeframe[]
	>({
		queryKey: ["timeframes", tenant.id],
		queryFn: async () => {
			const timeframes = await getAllTimeframes(tenant.id);
			if ("error" in timeframes) {
				console.error("Error fetching timeframes:", timeframes.error);
				throw new Error(timeframes.error);
			}
			return timeframes || [];
		},
	});
	...
	const transformObjectivesToOKRs = (): OKR[] => {
		...
		// Filter to only show team-level objectives
		const teamLevelObjectives = objectives.filter((obj) => obj.level === "team");

		return teamLevelObjectives.map((obj) => {
	...
	const teamOKRs = transformObjectivesToOKRs();

	// Filter objectives by team/user access - show team-level OKRs only
	const accessibleOKRs = teamOKRs.filter((okr) => {
		// Only show team-level objectives
		return okr.type === "team";
	});

	// Filter by status
	const activeOKRs = accessibleOKRs.filter((okr) => okr.status === "active");
	const pendingApprovalOKRs = accessibleOKRs.filter(
		(okr) => okr.status === "pending-approval"
	);
	const draftOKRs = accessibleOKRs.filter((okr) => okr.status === "draft");
	const completedOKRs = accessibleOKRs.filter(
		(okr) => okr.status === "completed"
	);
	...
	const calculateProgressPercentage = (
		...
		const progress = Math.min(
			100,
			Math.max(0, ((currentNum - startNum) / (targetNum - startNum)) * 100)
		);
		...
		return (
			<div className="container mx-auto p-6 flex justify-center items-center min-h-[50vh]">
				<div className="flex flex-col items-center">
					...
					<p className="text-neutral-600">Loading team objectives...</p>
				</div>
			</div>
		);
	}
		...
		<>
			<div className="container mx-auto p-6">
				<div className="flex justify-between items-center mb-6">
					<div>
						<h1 className="text-2xl font-bold text-neutral-900">Team OKRs</h1>
				...
				>
					<TabsList>
						...
						<TabsTrigger value="pending-approval">
							Pending Approval ({pendingApprovalOKRs.length})
						</TabsTrigger>
						<TabsTrigger value="drafts">Drafts ({draftOKRs.length})</TabsTrigger>
						<TabsTrigger value="completed">
							Completed ({completedOKRs.length})
						</TabsTrigger>
					</TabsList>
						...
						{activeOKRs.length > 0 ? (
							activeOKRs.map((okr) => (
								<Card
									...
									onClick={() => router.push(`${tenant.id}/objective/${okr.id}`)}
								>
									<CardHeader>
																	...
																	<div className="flex items-center mt-1 gap-3">
																		{kr.createdAt ? (
																			<div className="text-xs text-neutral-500 flex items-center">
																				...
																				{kr.createdAt.toLocaleDateString()}
																			</div>
																		) : null}
								...
								>
									...
									<CardContent>
										<div className="bg-purple-50 p-3 rounded-md text-sm text-purple-800 mb-4">
											This OKR is awaiting approval from your manager. You&apos;ll be
							...
							<Card className="shadow-sm">
								<CardContent className="flex flex-col items-center justify-center py-12">
									...
									<p className="text-neutral-500 mb-6">
										You don&apos;t have any OKRs awaiting approval.
									</p>
								</CardContent>
							</Card>
						)}
						...
						{draftOKRs.length > 0 ? (
							draftOKRs.map((draft) => (
								<Card
									...
									onClick={() => router.push(`/${tenant.id}/objectives/${draft.id}`)}
								>
									<CardHeader>
										<div className="flex justify-between items-start">
											<div>
									...
									<CardContent>
										...
										{draft.keyResults.length > 0 ? (
											...
											<p className="text-sm text-neutral-500">
												No key results created yet for this draft.
											</p>
										)}
									</CardContent>
								</Card>
							...
							<Card className="shadow-sm">
								<CardContent className="flex flex-col items-center justify-center py-12">
									...
									<p className="text-neutral-500 mb-6">
										You don&apos;t have any OKRs in draft status.
									</p>
									<Button
										onClick={() => router.push(`/${tenant.id}/create-objective`)}
						...
						{completedOKRs.length > 0 ? (
							...
							<Card className="shadow-sm">
								<CardContent className="flex flex-col items-center justify-center py-12">
									...
									<p className="text-neutral-500 mb-6">
										You don&apos;t have any completed OKRs yet.
									</p>
								</CardContent>
							</Card>
						)}
