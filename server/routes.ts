...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Get strategic pillars for current tenant (company-wide)
  app.get("/api/strategic-pillars", ensureAuthenticated, withTenant, async (req, res, next) => {
    try {
      const tenantId = req.tenantId;
      const pillars = await storage.getStrategicPillarsByTenant(tenantId);
      res.json(pillars);
    } catch (error) {
  ...
  // Get strategic pillars for a specific team
  app.get("/api/teams/:teamId/strategic-pillars", ensureAuthenticated, withTenant, async (req, res, next) => {
    try {
      const teamId = req.params.teamId;
      const pillars = await storage.getStrategicPillarsByTeam(teamId);
      res.json(pillars);
    } catch (error) {
      console.error("Error fetching team strategic pillars:", error);
      next(error);
    }
  });

  // Generate strategic pillars from company mission
  app.post("/api/strategic-pillars/generate", ensureAuthenticated, withTenant, async (req, res, next) => {
    try {
      const tenantId = req.tenantId;
      const { mission, teamId } = req.body;
      
      if (!mission) {
        return res.status(400).json({ error: "Mission statement is required" });
      }
      
      const pillars = await storage.generateStrategicPillarsFromMission(mission, tenantId, teamId);
      res.json(pillars);
    } catch (error) {
      console.error("Error generating strategic pillars:", error);
      next(error);
    }
  });

  // Create a new strategic pillar
  app.post("/api/strategic-pillars", ensureAuthenticated, withTenant, async (req, res, next) => {
    try {
      const tenantId = req.tenantId;
      const pillarData = {
        ...req.body,
      ...
      const pillar = await storage.createStrategicPillar(pillarData);
  ...
  app.put("/api/strategic-pillars/:id", ensureAuthenticated, withTenant, async (req, res, next) => {
    try {
      const pillarId = req.params.id;
      const updateData = req.body;
      
      const pillar = await storage.updateStrategicPillar(pillarId, updateData);
      res.json(pillar);
    } catch (error) {
  ...
  app.delete("/api/strategic-pillars/:id", ensureAuthenticated, withTenant, async (req, res, next) => {
    try {
      const pillarId = req.params.id;
