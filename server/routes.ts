...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.delete("/api/users/:id", ensureAuthenticated, withTenant, async (req, res, next) => {
    try {
      const userId = req.params.id;
      const currentUser = req.user as User;
      
      // Only admins or owners can delete users
      const userTenant = await db
        .select()
        .from(usersToTenants)
        .where(and(
      ...
      const userRole = userTenant[0].role;
      if (userRole !== 'admin' && userRole !== 'owner') {
        return res.status(403).json({ error: "Only admins or owners can delete users" });
      }
      ...
      // Verify the user being deleted belongs to the current tenant
      const targetUserInTenant = await db
        .select()
        .from(usersToTenants)
        .where(and(
      ...
      if (targetUserInTenant.length === 0) {
        return res.status(404).json({ error: "User not found in this tenant" });
      }
      
      // If the target user is an owner, only another owner can delete them
      const targetUserRole = targetUserInTenant[0].role;
      if (targetUserRole === 'owner' && userRole !== 'owner') {
        return res.status(403).json({ error: "Only an owner can delete another owner" });
      }
