...
export default function TenantSwitcher() {
  ...
  const onTenantSelect = useCallback((tenant: Tenant) => {
    ...
    toast({
      ...
      description: `Loading data for ${tenant.display_name || tenant.name || 'Organization'}...`,
      duration: 3000,
    });
          ...
          >
            {isLoading ? (
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="truncate">{selectedTenant.display_name || selectedTenant.name || 'Organization'}</span>
              </div>
            ) : (
              <div className="flex items-center gap-2">
                ...
                <CommandGroup>
                  {tenants?.map((tenant) => (
                    ...
                    >
                      <Avatar className="h-5 w-5 bg-slate-700">
                        <AvatarFallback className="text-xs text-slate-300">
                          {(tenant.display_name || tenant.name || '?').charAt(0)}
                        </AvatarFallback>
                      </Avatar>
                      <span>{tenant.display_name || tenant.name || 'Organization'}</span>
