          ...
          {isAddingTimeframe && (
            <Card className="border border-dashed">
              ...
              <CardFooter className="flex justify-between">
                <Button variant="outline" onClick={() => setIsAddingTimeframe(false)}>
                  Cancel
                </Button>
          ...
          {isLoadingTimeframes ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {existingTimeframes.map((timeframe) => (
                <Card key={timeframe.id} className="relative">
                  <CardHeader>
                    <CardTitle>{timeframe.name}</CardTitle>
                  </CardHeader>
                  <CardContent>
            ...
            <div className="text-center p-8 border rounded-md bg-gray-50">
              <p className="text-gray-500">No timeframes found. Switch to "Create Timeframes" tab to add some.</p>
            </div>
          )}
        </TabsContent>
      </Tabs>
