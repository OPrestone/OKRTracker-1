...
import { Save, User2, Building, Flag, CalendarRange, Settings, Bell, Lock, Mail, MoreHorizontal, Pencil, Trash, Trash2, Repeat, Calendar, Plus, Users, Server, Shield, Download, Eye, LayoutDashboard, Loader2, AlertTriangle } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogDescription,
                ...
                <CardFooter className="flex flex-col space-y-4 sm:flex-row sm:justify-between sm:items-center sm:space-y-0">
                  <div className="flex flex-col md:flex-row gap-4 items-center border rounded-lg p-3 bg-gray-50 w-full md:w-auto">
                    ...
                    <div className="flex-1 space-y-1">
                      <Label htmlFor="logo-upload" className="text-sm font-medium flex items-center">
                        Company Logo <span className="text-red-500 ml-1">*</span>
                      </Label>
                      <p className="text-xs text-gray-500">Max size: 2MB (Required)</p>
                      <Button
                        variant="outline"
                  ...
                  <div className="flex flex-col md:flex-row gap-4 items-center border rounded-lg p-3 bg-gray-50 w-full md:w-auto mt-4">
                    <div className="flex-shrink-0 w-16 h-16 bg-white rounded-lg border flex items-center justify-center overflow-hidden">
                      {systemLogoFile ? (
                        <img 
                          src={URL.createObjectURL(systemLogoFile)} 
                  ...
                  {!logoFile && (
                    <p className="text-sm text-red-500 mb-2">* Please upload a company logo to save settings</p>
                  )}
                  <Button
                    className="flex items-center gap-2"
                    onClick={handleSaveGeneralSettings}
                    disabled={isGeneralSaving || !logoFile}
                  >
                    {isGeneralSaving ? (
                      <>
                        <span className="animate-spin mr-2">⟳</span>
