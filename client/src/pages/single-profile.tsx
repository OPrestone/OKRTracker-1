...
export default function UserProfile() {
  ...
  // Fetch user details based on the URL id
  const { data: userDetails, isLoading: userLoading } = useQuery({
    queryKey: ["/api/users", id],
    enabled: !!id,
    queryFn: () => fetchUserById(id), // <-- make sure this is defined
  });
  const user = userDetails?.user;

  // Fetch user's objectives
  const { data: userObjectives, isLoading: objectivesLoading } = useQuery({
    queryKey: ["/api/users", id, "objectives"],
    enabled: !!id, // Query only if 'id' is available
  });
  
  // Fetch user's team
  const { data: userTeam, isLoading: teamLoading } = useQuery<Team>({
    queryKey: ["/api/teams", userDetails?.teamId],
    enabled: !!userDetails?.teamId
  });
